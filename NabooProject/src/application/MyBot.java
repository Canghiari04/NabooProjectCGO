...
public class MyBot extends TelegramLongPollingBot {
	...
	public void popolaFile(String nickName, String password) // Writer del file per individuare in un prossimo start
																// registrazioni di account gia' avvenute
	{
		try {
	...
	public void popolaDictionary(File f) // Allo start del bot telegram inserisce all'interno del dictionary tutti gli
											// account che abbiano gia'
	{ // effettuato la registrazione al bot, dato che ad ogni avvio viene
		// inizializzato il dictionary
	...
	public void clearFile(File f) // Usato principalmente per la pulizia del file di appoggio, (fileEliminate), a
									// (fileImport)
	{
		try {
	...
	public void copyFile(File fileOutput, File fileInput) // Copia del fileEliminate nel fileImport, su cui si base il
															// popolamento del dictionaryUtentes
	{
		try {
	...
	public void function(String str, SendMessage response, Update update) {
		...
		if (ch == '/') {
			function = str; // Salvo temporaneamente quale sia la funzione scelta precedentemente, in
							// maniera tale che possa successivamente
	...
	public void registrazione(SendMessage response, Update update) {
		try {
			...
			if (tokens.length != 2) {
				...
				if (dictionaryUtente.containsKey(nickName) && dictionaryUtente.containsValue(password)) // In caso
																										// dovesse
																										// essere
																										// presente un
																										// account con
				...
				{ // verra' richiesto nuovamento l'inserimento
					...
					reading = true; // Evidenzia la possibilita' che la lettura delle notizie possa avvenire
									// solamente con la propria registrazione

					popolaFile(nickName, password); // Aggiungo le nuove credenziali all'interno del file, per popolare
													// al prossimo avvio il dictionary
				}
	...
	public void accedi(SendMessage response, Update update) {
		try {
			...
			if (tokens.length != 2) // Condizione specificata per evitare scorretti inserimenti delle credenziale
			{
				...
				if (dictionaryUtente.containsKey(nickName) && dictionaryUtente.containsValue(password)) // Condizione
																										// per vericare
																										// se sia gia'
																										// avvenuta la
																										// registrazione
